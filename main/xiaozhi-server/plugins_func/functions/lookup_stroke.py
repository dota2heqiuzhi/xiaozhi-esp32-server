...
lookup_stroke_function_desc = {
    ...
    "function": {
        ...
        "description": (
            ...
            "═══════════════════════════════════════════════════════\n"
            "\n"
            "目的：让孩子在学写字的同时学一句小学课本古诗，扩展知识量。\n"
            "\n"
            "**铁律 4：必须是大众耳熟能详的古诗，不能给任何冷门诗**\n"
            "- 唯一允许的来源：小学/初中语文课本里的古诗\n"
            "- 唯一允许的作者范围：李白、杜甫、王之涣、王昌龄、孟浩然、白居易、苏轼、王维、贺知章、骆宾王、孟郊、张继、柳宗元、杜牧、李商隐、范仲淹、辛弃疾、陆游、岳飞、文天祥、毛泽东（外加少数同等知名度作者）\n"
            "- **绝对禁止**：清诗、明诗中的冷门作品；任何普通家长没听过的诗；冷僻字组成的诗（如'蟭龟'、'踟蹰停酒盏'这种）\n"
            "- 验证标准：如果一个 4 岁孩子的家长可能没听过这首诗，**就不要选**\n"
            "\n"
            "**铁律 5：诗句必须 2 句一对（12-20 个汉字），不能只给 1 句**\n"
            "- 古诗的对仗韵律必须配对才能让孩子感受到\n"
            "- 不要带书名号、作者名、解释 —— 只要诗句本身\n"
            "\n"
            "**铁律 6：必须真的包含 character 这个字面字**\n"
            "- 服务端会做校验，不含字的诗会被丢弃\n"
            "\n"
            "**铁律 7：同一个字每次轮换不同诗句，让孩子每次学到新东西**\n"
            "- 例如 character='月' 可在以下选项里随机轮换：\n"
            "    · '床前明月光，疑是地上霜'（李白·静夜思）\n"
            "    · '举头望明月，低头思故乡'（李白·静夜思）\n"
            "    · '海上生明月，天涯共此时'（张九龄）\n"
            "    · '明月几时有，把酒问青天'（苏轼）\n"
            ...
            "    · '小时不识月，呼作白玉盘'（李白）\n"
            "\n"
            "**铁律 8：找不到符合铁律 4-6 的诗，就把 poem_line 设为空字符串！**\n"
            "- 比如 character='龟'：小学课本古诗里几乎找不到含'龟'的诗 → **直接传 poem_line=''**\n"
            "- 比如 character='璎'、'麈' 等冷僻字 → **直接传 poem_line=''**\n"
            "- 服务端会自动 fallback 到精简版'这是X字'，**这比给一首冷门诗好得多**\n"
            "- 「宁可空，不可冷」\n"
            "\n"
            "═══════════════════════════════════════════════════════\n"
            "【完整示例】\n"
            ...
            "- '小朋友的有字怎么写' → character='友', context_phrase='小朋友', poem_line='桃花潭水深千尺，不及汪伦送我情'\n"
            "  （ASR 纠错：'有'(yǒu)→'友'(yǒu)。注意 character 必须改成'友'！）\n"
            "- '小鸟的鸟字怎么写' → character='鸟', context_phrase='小鸟', poem_line='月出惊山鸟，时鸣春涧中'\n"
            "  （'鸟'已在'小鸟'中，无需纠错；王维《鸟鸣涧》小学课本）\n"
            "- '春天的春字怎么写' → character='春', context_phrase='春天', poem_line='春眠不觉晓，处处闻啼鸟' 或 '春风又绿江南岸，明月何时照我还'（每次轮换）\n"
            "- '乌龟的龟字怎么写' → character='龟', context_phrase='乌龟', poem_line=''  ⚠️ **'龟'字在著名古诗里几乎不存在，必须传空，不要凑冷门诗！**\n"
            "- '璎字怎么写' → character='璎', context_phrase='', poem_line=''  ⚠️ 冷僻字\n"
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "poem_line": {
                    "type": "string",
                    "description": "**两句**包含 character 的小学/初中课本著名古诗（12-20 字）。冷门诗、不在课本的诗、不含此字的诗 = **必须传空字符串**。宁可空也不要给冷门诗。",
                },
            },
            "required": ["character"],
        },
